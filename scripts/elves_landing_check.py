...
def check_plan(plan_path: Path, session: dict[str, Any], report: Report) -> None:
    ...
    try:
        ...
        # Stable-ID mapping is mandatory whenever the plan contains B#-A# / M-A#
        # acceptance IDs — not only when evidence already carries IDs. Free-text
        # Acceptance criteria without IDs keep the criterion/checkbox path.
        has_stable_id_section = bool(
            re.search(r"(?im)^\s*[-*]\s+\[[ xX]?\]\s+(?:B\d+-A\d+|M-A\d+)\b", text)
            or re.search(r"(?im)\b(?:B\d+-A\d+|M-A\d+)\b", text)
            or "B1-A" in text
            or "M-A" in text
        )
        try:
        ...
        if has_stable_id_section or evidence_items or any(
            str(i.get("id", "")).startswith("M-A") for i in plan_items
        ):
